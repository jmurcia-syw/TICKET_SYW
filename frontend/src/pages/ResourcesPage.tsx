...
export default function ResourcesPage() {
  ...
  const columns: ColumnsType<Resource> = [
    ...
    { title: 'Email', dataIndex: 'email' },
    {
      ...
      render: (s: Skill[]) => s.map(sk => <Tag key={sk.id} color="blue">{sk.code}</Tag>),
    },
    { title: 'Estado', dataIndex: 'active', render: (v: boolean) => <Tag color={v ? 'green' : 'default'}>{v ? 'Activo' : 'Inactivo'}</Tag> },
    {
      ...
      render: (_: unknown, r: Resource) => (
        <Space>
          ...
          {isAdmin && r.active && <Tooltip title="Desactivar"><Button size="small" danger icon={<StopOutlined />} onClick={() => setConfirmDeactivate(r.id)} /></Tooltip>}
        </Space>
      ),
    },
  ]
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, gap: 8 }}>
        ...
        {isAdmin && <Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>Nuevo recurso</Button>}
      </div>
