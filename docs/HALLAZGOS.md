# Hallazgos — SyWork Desk (alpha)

> Registro de bugs y observaciones de las pruebas de QA.
> Plan de pruebas: [PLAN_PRUEBAS.md](PLAN_PRUEBAS.md).
> Orden: por número (H-NNN). Ver la tabla de índice para severidad y estado.

---

## Índice

| # | Título | Severidad | Módulo | Estado |
|---|--------|-----------|--------|--------|
| [H-001](#h-001) | JWT inválido devuelve 500 en vez de 401 en maestros | Alto | Auth · Maestros | Abierto |
| [H-002](#h-002) | Nombre de cliente sin validación de caracteres | Bajo | Clientes | Abierto |
| [H-003](#h-003) | Email solo valida formato, no existencia | Medio | Clientes | Abierto (producto) |
| [H-004](#h-004) | Teléfono sin selector de código de país | Medio | Clientes | Abierto |
| [H-005](#h-005) | Teléfono sin longitud máxima | Medio | Clientes | Abierto |
| [H-006](#h-006) | Campos VPN visibles en texto plano al crear/editar | Alto | Clientes (seguridad) | Abierto |
| [H-007](#h-007) | Falta feedback claro cuando la validación falla | Medio | Global (formularios) | Abierto |
| [H-008](#h-008) | Editar Proyecto ignora el cambio de Cliente en silencio | Alto | Proyectos | Abierto (producto) |
| [H-009](#h-009) | Identificación acepta cualquier carácter y longitud | Medio | Equipo | Abierto |
| [H-010](#h-010) | Nacionalidad es input libre en vez de lista de países | Medio | Equipo | Abierto |
| [H-011](#h-011) | Fecha de nacimiento sin validación de edad mínima | Medio | Equipo | Abierto |
| [H-012](#h-012) | Nivel de estudios es input libre en vez de catálogo | Bajo | Equipo | Abierto |
| [H-013](#h-013) | Equipo es input libre en vez de catálogo administrable | Bajo | Equipo | Abierto |
| [H-014](#h-014) | Matriz de permisos omite 9 permisos reales | Alto | Roles y Permisos | Abierto |
| [H-015](#h-015) | Ticket se puede cerrar sin tiempo registrado | Alto | Tickets · Cronómetro | Abierto (producto) |
| [H-016](#h-016) | Múltiples usuarios simultáneos en el mismo navegador | Medio/Alto | Auth | Abierto (producto) |
| [H-017](#h-017) | Listados de tickets sin ordenamiento útil | Medio | Tickets · UX | Abierto (producto) |

**Resumen por severidad**: Alto ×5 (H-001, H-006, H-008, H-014, H-015) · Medio ×8 · Bajo ×3 · Mixto ×1 (H-016).

---

<details>
<summary><strong>Template de hallazgo</strong> (copiar y llenar)</summary>

```markdown
### H-NNN — Título corto
- **Severidad**: Crítico | Alto | Medio | Bajo | Nota
- **Módulo**: Tickets | Tareas | Listas | Tiempos | Auth | Kanban | Notificaciones | Maestros
- **Rol usado**: Admin | Coordinador | QM | Resolutor | Encargado
- **Fecha**: YYYY-MM-DD
...
**Esperado**:
**Observado**:
**Evidencia**: screenshot / log / request-id
**Referencia**: spec / FR / escenario relacionado
**Estado**: Abierto | En revisión | Corregido en commit `xxxxxxx` | No reproducible
```
...
**Fix propuesto**: `Input maxLength={20}` + regla `{ pattern: /^\+?[0-9\s\-()]{7,20}$/ }` como parche mínimo, o resolver junto con [H-004](#h-004) usando componente E.164.
**Referencia**: [ClientsPage.tsx:170](frontend/src/pages/ClientsPage.tsx).
**Estado**: Abierto
...
- Cuando ocurre un 500 (ver [H-001](#h-001)), el mensaje se pierde y solo aparece "Error al guardar" genérico o nada.
...
**Nota**: el commit `523f91c` (spec 013 — normalizador global de errores) puede haber resuelto parte de esto. Verificar tras el rebuild.
...
**Estado**: Abierto (verificar contra spec 013)
...
- Combina mal con [H-007](#h-007): no hay feedback de "campo no editable".
