...
- [X] T009 [US3] Verificar en UI el caso "ticket no asignado a este proyecto": provocar la operación y confirmar toast con el mensaje específico del servidor; si el mensaje del endpoint no es apto para usuario final, ajustar SOLO ese texto en la ruta correspondiente de `backend/api/routes/` (cambio de string, no de lógica) *(Verificado contra Docker real 2026-07-10: login Resolutor, ticket TK-000001 sin asignar, click "Iniciar" cronómetro → POST /api/timer/start 403 `NOT_ASSIGNED`, toast visible "El recurso no participa de este ticket", pantalla no congelada. Mensaje ya apto para usuario final, sin cambios de texto necesarios.)*
- [X] T010 [US3] Verificar en UI el caso "usuario sin permisos": con rol limitado (p. ej. Resolutor en acción restringida), confirmar toast 403 con mensaje entendible y pantalla no congelada *(Verificado contra Docker real 2026-07-10: Resolutor sin `tickets:edit` — el frontend ya oculta el editor de Skills para este rol (defensa en profundidad); se disparó `ticketService.updateTicketSkills` desde la sesión autenticada del Resolutor → PATCH /api/tickets/{id}/skills 403 `FORBIDDEN`, toast "Acceso denegado" visible, UI interactiva tras el error.)*
- [X] T011 [US3] Verificar en UI el caso "proyecto no encontrado": operar sobre proyecto inexistente/eliminado y confirmar toast 404 con mensaje específico *(Verificado contra Docker real 2026-07-10: Admin navega a /projects/00000000-0000-0000-0000-000000000000/people → 3 llamadas GET 404 `NOT_FOUND`, una sola notificación toast "Proyecto no encontrado" visible (dedupe funcionando), navegación no congelada.)*
